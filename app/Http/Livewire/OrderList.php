...
class OrderList extends Component
{
    ...
    public function render()
    {
        ...
        $masterPlanBefore = MasterPlan::selectRaw("MAX(id) id")->
            ...
            ")->
            where("master_plan.cancel", "N")->where("tgl_plan", "<", $this->date)->
        ...
        $this->orders = DB::table('master_plan')
            ->selectRaw("
                ...
                COALESCE(output_finishline.progress, 0) as target,
                CONCAT(masterproduct.product_group, ' - ', masterproduct.product_item) as product_type
            ")
            ->leftJoin('act_costing', 'act_costing.id', '=', 'master_plan.id_ws')
            ->leftJoin('so', 'so.id_cost', '=', 'act_costing.id')
            ...
            ->leftJoin(
                ...
                "),
                function ($join) {
                    $join->on("output_finishline.sewing_line", "=", "master_plan.sewing_line");
                    $join->on("output_finishline.id_ws", "=", "master_plan.id_ws");
                    $join->on("output_finishline.tgl_plan", "=", "master_plan.tgl_plan");
                }
            )
            ->where('so_det.cancel', 'N')
            ->where('master_plan.cancel', 'N')
