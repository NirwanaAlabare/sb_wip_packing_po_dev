...
@section('custom-script')
    <script>
        ...
        Livewire.on('preSubmitReject', (rejectId, department, rejectSize, rejectType, rejectArea, rejectImage, rejectX, rejectY) => {
            Swal.fire({
                icon: 'info',
                title: 'Masukkan REJECT ini?',
                html: `<table class="table text-start w-auto mx-auto">
                            <tr>
                                <td>ID<td>
                                <td>:<td>
                            ...
                            <tr>
                                <td>Dept<td>
                                <td>:<td>
                                <td>`+(department == "packing" ? "finishing" : department)+`<td>
                            <tr>
                            <tr>
                                <td>Size<td>
                                <td>:<td>
                            ...
                            <tr>
                                ...
                                <td>:<td>
                                <td>
                                    <button type="button" class="btn btn-dark" onclick="onShowDefectAreaImage('`+rejectImage+`', '`+rejectX+`', '`+rejectY+`')">
                                        <i class="fa-regular fa-image"></i>
                                    </button>
                                <td>
                            <tr>
                ...
                if (result.isConfirmed) {
                    Livewire.emit('submitReject', rejectId, department);
                } else if (result.isDenied) {
                    Swal.fire({
                        icon: 'info',
                        title: 'Submit REJECT dibatalkan',
        ...
        Livewire.on('preCancelReject', (rejectId, rejectSize, rejectType, rejectArea, rejectImage, rejectX, rejectY, department) => {
            Swal.fire({
                icon: 'warning',
                title: 'Cancel REJECT?',
                html: `<table class="table text-start w-auto mx-auto">
                ...
                showConfirmButton: true,
                showDenyButton: true,
                confirmButtonText: 'Cancel REJECT',
                confirmButtonColor: '#fa4456',
                denyButtonText: 'Batal',
            }).then((result) => {
                if (result.isConfirmed) {
                    Livewire.emit('cancelReject', rejectId);
                } else if (result.isDenied) {
                    Swal.fire({
                        icon: 'info',
                        title: 'Pengembalian REJECT dibatalkan',
                        confirmButtonText: 'Ok',
                        confirmButtonColor: '#fa4456',
                    });
                }
